# -*- coding: utf-8 -*-
# This is learning model for rates images
...
class RatesLearningModel(UWOLearningModel):
    """Learning model class for rates images - 26x13 size
    """
    def __init__(self, traning_data_dir, out_model_dir, out_label_path):
        ...
        for i in range(511):
            self.labels += [str(i)]
        self.catetory = len(self.labels)

    def get_labels(self):
        return self.labels

    def run_machine_learning(self):
        """Use DNN model to learn because it's less output size.. :)
        """
    ...
    def __run_machine_learning_cnn(self):
        ...
        X_shaped = tf.reshape(X, [-1, 26, 13, 3])
        Y = tf.placeholder(tf.float32, [None, self.catetory], name="Y")
        ...
        W3 = tf.Variable(tf.random_normal([13 * 7 * 4, self.catetory * 4], stddev=0.01))
        L3 = tf.reshape(L1, [-1, 13 * 7 * 4])
        L3 = tf.matmul(L3, W3)
        L3 = tf.nn.relu(L3)
        ...
        for i in range(len(files)):
            ...
            if result == 0.0:
                determined = sess.run(tf.argmax(model, 1),
                                      feed_dict={X: x_data, Y: y_data, keep_prob: 1})
                print("(Wrong)", files[i], ": Determined", determined)
            sum += result
